...
class MQTTPublisher:
    ...
    def _send_sms_via_gammu(self, number, text, unicode_mode=None, flash_mode=False):
        ...
        try:
            ...
            if flash_mode:
                logger.info(
                    f"⚡ Sending Flash SMS (will display on screen without saving)"
                )
            ...
            logger.info(
                f"📊 SMS counter incremented by {len(messages)} to: {self.sms_counter.get_count()}"
            )
    ...
    def _handle_flash_button_sms_send(self):
        ...
        logger.info(
            f"Flash button pressed - current state: phone='{self.current_phone_number}', message='{self.current_message_text}'"
        )
        ...
        if (
            ...
            if self.connected:
                ...
                self.client.publish(status_topic, json.dumps(status_data), retain=False)
            logger.warning(
                f"Flash button pressed but fields empty: phone='{self.current_phone_number}', message='{self.current_message_text}'"
            )
            return

        # Send Flash SMS using current values
        logger.info(
            f"Flash Button SMS send: {self.current_phone_number} -> {self.current_message_text}"
        )
        if hasattr(self, "gammu_machine") and self.gammu_machine:
    ...
    def _publish_discovery_configs(self):
        ...
        discoveries = [
            ("homeassistant/sensor/sms_gateway_signal_2/config", signal_config),
            ("homeassistant/sensor/sms_gateway_network_2/config", network_config),
            ("homeassistant/sensor/sms_gateway_last_sms_2/config", sms_config),
            (
                "homeassistant/sensor/sms_gateway_send_status_2/config",
                send_status_config,
            ),
            (
                "homeassistant/sensor/sms_gateway_delete_status_2/config",
                delete_status_config,
            ),
            (
                "homeassistant/sensor/sms_gateway_modem_status_2/config",
                device_status_config,
            ),
            (
                "homeassistant/sensor/sms_gateway_sent_count_2/config",
                sms_counter_config,
            ),
            ("homeassistant/sensor/sms_gateway_modem_imei_2/config", modem_imei_config),
            (
                "homeassistant/sensor/sms_gateway_modem_model_2/config",
                modem_model_config,
            ),
            ("homeassistant/sensor/sms_gateway_sim_imsi_2/config", sim_imsi_config),
            (
                "homeassistant/sensor/sms_gateway_sms_capacity_2/config",
                sms_capacity_config,
            ),
            ("homeassistant/button/sms_gateway_send_button_2/config", button_config),
            (
                "homeassistant/button/sms_gateway_send_flash_button_2/config",
                flash_button_config,
            ),
            (
                "homeassistant/button/sms_gateway_reset_counter_2/config",
                reset_counter_button_config,
            ),
            (
                "homeassistant/button/sms_gateway_delete_all_sms_2/config",
                delete_all_sms_button_config,
            ),
            ("homeassistant/text/sms_gateway_phone_number_2/config", phone_text_config),
            (
                "homeassistant/text/sms_gateway_message_text_2/config",
                message_text_config,
            ),
        ]
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=1) as executor:
                ...
                try:
                    ...
                    self.device_tracker.record_failure(
                        f"{operation_name}: Python timeout (60s)"
                    )
                    self.publish_device_status()
                    logger.error(
                        f"⏱️ Gammu operation '{operation_name}' timed out after 60s"
                    )
                    raise TimeoutError(
                        f"Gammu operation '{operation_name}' timed out after 60s"
                    )
                except Exception as e:
    ...
    def publish_initial_states_with_machine(self, gammu_machine):
        ...
        try:
            ...
            try:
                smsc_info = self.track_gammu_operation(
                    "GetSMSC", gammu_machine.GetSMSC, Location=1
                )
                smsc_number = smsc_info.get("Number", "Not set")
                smsc_name = smsc_info.get("Name", "Unknown")
                logger.info(
                    f"📞 SMSC Location 1: Number='{smsc_number}', Name='{smsc_name}'"
                )
            except Exception as e:
